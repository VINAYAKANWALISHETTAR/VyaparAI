...
class FinancialService:
    ...
    def get_insights(self, user_id: str):
        ...
        insights.extend(self._check_revenue_trends(business_ids))
        insights.extend(self._check_expense_trends(business_ids))
        insights.extend(self._check_receivable_alerts(business_ids))
        insights.extend(self._check_cash_flow_warnings(user_id))
        insights.extend(self._check_anomaly_flags(business_ids))
    ...
    def _check_revenue_trends(self, business_ids: list[str]):
        insights = []
        today = date.today()
    ...
    def _check_expense_trends(self, business_ids: list[str]):
        insights = []
        today = date.today()
        start_of_month = today.replace(day=1)
    ...
    def _check_cash_flow_warnings(self, user_id: str):
        insights = []
        cash_flow = self.get_cash_flow(user_id)
    ...
    def _check_anomaly_flags(self, business_ids: list[str]):
        insights = []
        anomalies = self._check_duplicate_invoices(business_ids) + self._check_overpayments(business_ids) + self._check_unusual_expenses(business_ids)
        if anomalies:
            insights.append({
                "type": "anomaly_flag",
